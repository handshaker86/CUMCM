...
def jacobian_d_numeric(d_um, p, wv, angle_deg, is_multibeam=True, delta_um=1e-3):
    Rp = (
        calculate_reflectance_thin_film(
            np.insert(p, 0, d_um + delta_um), wv, angle_deg, is_multibeam
        ) * 100.0
    )
    Rm = (
        calculate_reflectance_thin_film(
...
def jacobian_params_numeric(d_um, p, wv, angle_deg, is_multibeam=True, eps_vec=None):
    ...
    for k in range(K):
        ...
        Rp = (
            calculate_reflectance_thin_film(
                np.insert(p + dp, 0, d_um), wv, angle_deg, is_multibeam
            ) * 100.0
        )
        Rm = (
            calculate_reflectance_thin_film(
...
def fit_optical_params(
    exp_wavenumbers, exp_reflectance_percent, angle_deg, fixed_d_um, is_multibeam=True
):
    ...
    def loss_p(p):
        mod = (
            calculate_reflectance_thin_film(
                np.insert(p, 0, fixed_d_um), wv, angle_deg, is_multibeam
            )
            * 100.0
        )
    ...
    res = minimize(
        loss_p, p0, method="L-BFGS-B", bounds=bounds, options={"disp": False}
    )
    p_hat = res.x

    R_hat = (
        calculate_reflectance_thin_film(
            np.insert(p_hat, 0, fixed_d_um), wv, angle_deg, is_multibeam
        )
        * 100.0
    )
    residuals = y - R_hat
    ...
    Jp = jacobian_params_numeric(
        fixed_d_um, p_hat, wv, angle_deg, is_multibeam, eps_vec=None
    )
    JTJ = Jp.T @ Jp
    try:
...
def fit_thickness_and_uncertainty(
    wv,
    R_percent,
    angle_deg,
    p_fixed,
    ...
    bounds_um=(8.0, 10.0),
    delta_um=1e-3,
    Sigma_p_for_propagation=None,
    sigma_R_sq_from_fit=None,
):
    ...
    def loss_d(d_um):
        mod = (
            calculate_reflectance_thin_film(
                np.insert(p_fixed, 0, d_um), wv, angle_deg, is_multibeam
            )
            * 100.0
        )
        return np.mean((mod - y) ** 2)
    ...
    R_hat = (
        calculate_reflectance_thin_film(
            np.insert(p_fixed, 0, d_hat), wv, angle_deg, is_multibeam
        ) * 100.0
    )
    residuals = y - R_hat
    N = len(y)
    ...
    sigma_R_sq_hat = (
        ...
        if sigma_R_sq_from_fit is None
        else float(sigma_R_sq_from_fit)
    )
    ...
    CI95_total = None
    if (
        (Sigma_p_for_propagation is not None)
        and np.all(np.isfinite(Sigma_p_for_propagation))
        and np.isfinite(sumJ2)
        and (sumJ2 > 0)
    ):
        Jp = jacobian_params_numeric(
            d_hat, p_fixed, wv, angle_deg, is_multibeam, eps_vec=None
        )  
        A = sumJ2
        B = Jd[None, :] @ Jp  
        middle = B @ Sigma_p_for_propagation @ B.T  
...
def plot_band_contrib_grouped(
    wv10, J10, wv15, J15, bands=BANDS, title="Information Share by Band", savepath=None
):
...
if __name__ == "__main__":
    ...
    p10, Sigma_p10, sigmaR10, rep10 = fit_optical_params(
        wv1, R1, ang1, D_FIX, is_multibeam=IS_MULTIBEAM
    )
    p15, Sigma_p15, sigmaR15, rep15 = fit_optical_params(
        wv2, R2, ang2, D_FIX, is_multibeam=IS_MULTIBEAM
    )
    ...
    d10, rpt10 = fit_thickness_and_uncertainty(
        ...
        bounds_um=BOUNDS_UM,
        delta_um=DELTA_UM,
        Sigma_p_for_propagation=Sigma_p_avg,
        sigma_R_sq_from_fit=sigmaR_avg,
    )
    ...
    if (w10c + w15c) > 0:
        ...
        print(
            f"[条件] d̂ = {d_cond:.6f} μm | 95%CI|p = [{d_cond - z*SEc:.6f}, {d_cond + z*SEc:.6f}] μm"
        )
    else:
        print("[条件] 权重为 0，无法合并。")
    ...
    if (rpt10.get("SE_total") is not None) and (rpt15.get("SE_total") is not None):
        ...
        if np.isfinite(se10_t) and np.isfinite(se15_t):
            w10t = 1.0 / (se10_t**2 + 1e-18)
            w15t = 1.0 / (se15_t**2 + 1e-18)
            d_tot = (w10t * d10 + w15t * d15) / (w10t + w15t)
            SEt = np.sqrt(1.0 / (w10t + w15t))
            print(
                f"[总计] d̂ = {d_tot:.6f} μm | 95%CI_total = [{d_tot - z*SEt:.6f}, {d_tot + z*SEt:.6f}] μm"
            )
        else:
    ...
    # 计算灵敏度谱
    Jd10 = jacobian_d_numeric(
        d10, p_avg, wv1, ang1, is_multibeam=IS_MULTIBEAM, delta_um=DELTA_UM
    )
    Jd15 = jacobian_d_numeric(
        d15, p_avg, wv2, ang2, is_multibeam=IS_MULTIBEAM, delta_um=DELTA_UM
    )
