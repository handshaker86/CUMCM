...
from q3_method import get_silicon_refractive_index_sellmeier
...
def get_sic_refractive_index_LD(wavenumbers_cm, params):
    ...
    i = 1j
    lorentz_term = eps_inf * (sigma**2 - sig_LO**2 + i*gamma_phonon*sigma) / (sigma**2 - sig_TO**2 + i*gamma_phonon*sigma)
...
def get_sic_refractive_index_high(sigma, params_low, drude_scale=0.0):
    ...
    return get_sic_refractive_index_LD(sigma, [eps_inf, sig_TO, sig_LO, gamma_ph, sig_p*drude_scale, gamma_e])
...
def get_sic_substrate_n(sigma):
    ...
    eps_sell = n_sell**2
    sigma_safe = sigma + 1e-9
    drude_term = (fixed_drude[0]**2) / (sigma_safe*(sigma_safe + 1j*fixed_drude[1]))
...
# 5) 薄膜反射率（混合模型）
# -----------------------------
def calculate_reflectance_hybrid(d_um, params_low, sigma, theta_deg, split=1500.0, tw=120.0, drude_scale_high=0.0):
...
def fit_thickness_high(sigma, R_exp, theta_deg, split=1500.0, tw=120.0):
    ...
    def obj_d(d):
        R_model = calculate_reflectance_hybrid(d, dummy_params_low, sigma_high, theta_deg,
                                               split=split, tw=tw, drude_scale_high=0.0)
        return np.mean((R_model*100 - R_high)**2)

    res = minimize_scalar(obj_d, bounds=(1.0,10.0), method='bounded')
    return res.x

def refine_highfreq_thickness(sigma, R_exp, theta_deg, d_init, params_low, split=1500.0, tw=120.0):
    sigma_high = sigma[sigma>=split]
    R_high = R_exp[sigma>=split]

    def obj(d):
        R_model = calculate_reflectance_hybrid(d, params_low, sigma_high, theta_deg,
                                               split=split, tw=tw)
        return np.mean((R_model*100 - R_high)**2)
...
def fit_lowfreq_params(sigma, R_exp, theta_deg, d_fixed, split=1500.0, tw=120.0):
    def obj_low(params):
        R_model = calculate_reflectance_hybrid(d_fixed, params, sigma, theta_deg,
                                               split=split, tw=tw)
    ...
    bounds = [(5.0,20.0),(60.0,810.0),(800.0,1000.0),(0.5,100.0),(0.0,200.0),(1.0,1000.0)]
...
# 8) Step 3: 全波段微调 + 高频加权
# -----------------------------
def fit_full_hybrid(sigma, R_exp, theta_deg, d_init, params_low_init, split=1500.0, tw=120.0, highweight=5.0):
    def obj_full(fit_params):
        d = fit_params[0]
        params_low = fit_params[1:]
        R_model = calculate_reflectance_hybrid(d, params_low, sigma, theta_deg,
                                               split=split, tw=tw)
    ...
    bounds = [(3,4)] + [(5.0,20.0),(60.0,810.0),(800.0,1000.0),(0.5,100.0),(0.0,200.0),(1.0,1000.0)]
...
if __name__ == "__main__":
    ...
    # 可视化拟合
    for wn, R_exp, angle, label, color in [(wn1,R1,10,"10°","red"),(wn2,R2,15,"15°","green")]:
        R_fit = calculate_reflectance_hybrid(final_d, final_params_low, wn, angle,
                                            split=split, tw=tw)*100
        plt.figure(figsize=(10,6))
        plt.plot(wn,R_exp,label=f"实验 ({label})",alpha=0.7)
        plt.plot(wn,R_fit,label=f"拟合 ({label})",color=color,linewidth=2)
        plt.xlabel("Wavenumber (cm⁻¹)")
        plt.ylabel("Reflectance (%)")
        plt.title(f"{label} 最终拟合")
        plt.legend()
        plt.grid(True)
        plt.show()

    # 可视化 n/k
    sigma_plot = np.linspace(400,4000,1200)
    N_low = get_sic_refractive_index_LD(sigma_plot, final_params_low)
    N_high = get_sic_refractive_index_high(sigma_plot, final_params_low, drude_scale=0.0)
    w_low = cosine_blend_weights(sigma_plot, split, tw)
    N_blend = w_low*N_low + (1-w_low)*N_high
    n_fit = N_blend.real
    k_fit = N_blend.imag

    fig, ax1 = plt.subplots(figsize=(12,6))
    ax1.plot(sigma_plot, n_fit,label="n")
    ax1.set_xlabel("Wavenumber (cm⁻¹)")
    ax1.set_ylabel("n")
    ax1.grid(True,linestyle="--",alpha=0.4)
    ax2 = ax1.twinx()
    ax2.plot(sigma_plot, k_fit, linestyle="--",label="k")
    ax2.set_ylabel("k")
    lines1, labels1 = ax1.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    ax1.legend(lines1+lines2, labels1+labels2, loc="best")
    ax1.set_title("最终 4H-SiC 光学常数 (混合模型)")
    plt.tight_layout()
    plt.show()
